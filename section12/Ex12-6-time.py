...
#time.time(): Unix timestamp 반환 (1970년 1월1일 부터 초)
...
now = datetime.now() #line 17
print(f'현재 날짜/시간: {now}')
print(f'현재 날짜/시간: {now.strftime("%Y년 %m월 %d일 %H:%M:%S")}')
y = datetime.now().year
...
print(f'내일:{tomorrow}, 어제:{yesterday}, 오늘:{today}')
...
while True:
    ...
    if sec == 10:
        break
    time.sleep(0.5 ) #1초 동안 프로그램 실행 중지
    sec += 1
